...
# Answer: Rails generate command executed in terminal; db created; migrate command executed
...
movies.each do |movie|
    puts "#{movie.title.ljust(30)} #{movie.year_released.to_s.ljust(15)} #{movie.rated.ljust(10)} #{movie.studio.name}"
  end
...
roles = Role.includes(:movie, :actor).order('movies.title ASC').all
roles.each do |role|
    puts "#{role.movie.title.ljust(30)} #{role.actor.name.ljust(25)} as #{role.character_name}"
  end